...
const navigationGroups = [
  ...
  },
  {
    title: 'การเทรด',
    items: [
      { to: '/admin/trading-products', label: 'ผลิตภัณฑ์การเทรด', icon: TrendingUp, badge: null },
      { to: '/admin/forex', label: 'Forex Trading', icon: BarChart3, badge: null },
      { to: '/admin/crypto', label: 'Cryptocurrency', icon: Coins, badge: null },
      { to: '/admin/stocks', label: 'Stock Trading', icon: Activity, badge: null },
      { to: '/admin/metals', label: 'Precious Metals', icon: Gem, badge: null },
    ]
  },
  {
    title: 'ลูกค้า & การเงิน',
    items: [
      { to: '/admin/clients', label: 'ลูกค้า', icon: Users, badge: '89' },
      { to: '/admin/transactions', label: 'รายการเทรด', icon: ShoppingCart, badge: '156' },
      { to: '/admin/ib-program', label: 'IB Program', icon: Award, badge: 'New' },
      { to: '/admin/payments', label: 'การชำระเงิน', icon: CreditCard, badge: null },
    ]
  },
  {
    title: 'เนื้อหา & การตลาด',
    items: [
      { to: '/admin/banners', label: 'แบนเนอร์', icon: ImageIcon, badge: null },
      { to: '/admin/hero-slides', label: 'สไลด์หน้าแรก', icon: Layers, badge: null },
      { to: '/admin/promotions', label: 'โปรโมชั่น', icon: Megaphone, badge: '3' },
      { to: '/admin/news', label: 'ข่าวสาร', icon: FileText, badge: null },
      { to: '/admin/reviews', label: 'รีวิว', icon: Star, badge: '12' },
    ]
  },
  {
    title: 'การตั้งค่า',
    items: [
      { to: '/admin/settings', label: 'การตั้งค่า', icon: Settings, badge: null },
...
const AdminSidebar = () => {
  ...
  return (
    <aside className={`hidden md:flex flex-col h-screen bg-gradient-to-b from-red-50 via-white to-red-50 border-r border-red-200/50 transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}>
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-red-200/50 bg-white/80 backdrop-blur-sm">
        {!isCollapsed && (
          <NavLink to="/admin" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              <span className="font-bold text-lg text-gray-900 group-hover:text-red-600 transition-colors">RubyFX Admin</span>
              <span className="text-xs text-gray-500">ระบบจัดการการเทรด</span>
            </div>
          </NavLink>
        )}
        {isCollapsed && (
        ...
        <div className="p-4 border-b border-red-200/50">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-200 focus:border-red-300 transition-colors"
            />
          </div>
        </div>
      )}
          ...
          {navigationGroups.map((group, groupIndex) => (
            <div key={group.title}>
              ...
              {groupIndex < navigationGroups.length - 1 && !isCollapsed && (
                <div className="my-4 h-px bg-red-200/50" />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="border-t border-border/50 p-4">
          <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50 hover:bg-muted transition-colors cursor-pointer group">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground truncate">admin@rubyfx.com</p>
            </div>
            ...
            <Dialog>
              ...
              <DialogContent className="sm:max-w-[440px]">
                ...
                <div className="grid gap-3">
                  ...
                  <a href="mailto:info@rubyfx.com" className="w-full">
